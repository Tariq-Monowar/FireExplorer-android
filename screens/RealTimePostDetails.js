...
const RealTimePostDetails = ({route}) => {
  ...
  const deletePost = async () => {
    try {
      const res = await realtimeContext.deletePost(post.id, post.image);
      if (res) {
        navigate.goBack();
      }
    } catch (error) {
  ...
  return (
    <>
      ...
      />
      {realtimeContext.findDataLoading && <Loading />}
      <ScrollView>
        <View
...
export default RealTimePostDetails;
